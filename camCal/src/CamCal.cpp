...
void CCamCal::calCamEdaOpt(void)
{
	...
	saveCamMat(&oCamParam, m_oVr, m_oVl);
}
	...
	while (true)
	{
		...
		if (voPt.size())
		{
			for (int i = 0; i < voPt.size(); i++)
			{
				...
				if (tstStGrdPt(oStGrdPt, poCamParam)) { std::cout << "check" << std::endl;bStGrdPtFlg = true; break; }
...
void CCamCal::saveCamMat(CCamParam* poCamParam, cv::Point2f oVr, cv::Point2f oVl){
...
void CCamCal::plt3dGrd(CCamParam* poCamParam, cv::Point2f oVr, cv::Point2f oVl, cv::Point oStGrdPt)
{
	...
	cv::namedWindow("3D grid on ground plane", WINDOW_NORMAL);
...
std::vector<cv::Point> CVanLnSel::process(void)
{
	...
	if (m_oCfg.getCalSelVanLnFlg())
	{
		...
		cv::namedWindow("selector of vanishing lines", WINDOW_NORMAL);
...
void CVanLnSel::addNd(int nX, int nY)
{
	...
	if (1 == m_voNd.size() % 2)
	{
		...
		cv::circle(m_oImgBg, oCurrNd, 6, cv::Scalar(255, 0, 0), 1, LINE_AA);  // draw the circle
		cv::line(m_oImgBg, oPrevNd, oCurrNd, cv::Scalar(255, 255, 255), 2, LINE_AA);
		cv::imshow("selector of vanishing lines", m_oImgBg);
