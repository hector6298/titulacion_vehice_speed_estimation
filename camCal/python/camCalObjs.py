...
class CCamParam(object):
    def __init__(self):
        ...
        self.m_afP = np.empty((12,), np.float32)
    ...
    def setInParamMat(self,fFx:float, fFy:float, fCx:float, fCy:float):
        self.m_afK[0] = fFx
        self.m_afK[1] = 0.0
        self.m_afK[0] = fCx
        self.m_afK[1] = 0.0
    ...
    def setRotMat(self, fRoll:float, fPitch:float, fYaw:float):
        if COORD_SYS_TYP == 0:
    ...
    def setTntMat(self, fTx:float, fTy:float, fTz:float):
        self.m_afT[0] = fTx
        self.m_afT[1] = fTy
        self.m_afT[2] = fTz
    ...
    def readCamInParamTxt(self, CamInParamPath:str):
        pass

    def initCamMdl(self, sparamRng:SParamRng):
...
class CCfg(object):
    def __init__(self, 
        ...
        assert len(lineDists) == len(linePoints), \
